...
#' @param step the number of markers handled at a time, smaller value would reduce the memory cost
...
function(
    ...
    logging.log(paste0("Computing GRM for ", n, " individuals with a step of ", step), "\n", verbose = verbose)
    
    K <- try(kin_cal(M@address, geno_ind = ind_idx, threads = cpu, step = step, verbose = verbose, mkl = HPCMathLib), silent=TRUE)
