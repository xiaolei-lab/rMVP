...
function(y, X, K, ngrids=100, llim=-10, ulim=10, esp=1e-10) {
    ...
    m <- length(logdelta)
    delta <- exp(logdelta)
    Lambdas <- matrix(eig.R$values, df, m) + matrix(delta, df, m, byrow = TRUE)
    Etasq <- matrix(etas * etas, df, m)
    LL  <- 0.5 * (
        df * (log(df/(2 * pi)) - 1 - log(colSums(Etasq / Lambdas)))
        - colSums(log(Lambdas))
    )
    dLL <- 0.5 * delta * (
        df * colSums(Etasq / (Lambdas * Lambdas)) 
        / colSums(Etasq / Lambdas) 
        - colSums(1 / Lambdas)
    )
    ...
    for (i in seq_len(m - 1)) {
        if ((dLL[i] * dLL[i + 1] < 0) && (dLL[i] > 0) && (dLL[i + 1] < 0)) {
            ...
            # LL
            LL <- emma.delta.REML.LL.wo.Z(r$root, eig.R$values, etas)
            optLL <- append(optLL, LL)
        }
    }
    ...
    result <- list(REML = maxLL, delta = maxdelta, ve = maxve, vg = maxva)
    return(result)
...
emma.eigen.R.wo.Z <- function(K, X) {
    ...
    result <- list(
        values = eig$values[seq_len(n - q)] - 1, 
        vectors = eig$vectors[, seq_len(n - q)]
    )
    return(result)
}
