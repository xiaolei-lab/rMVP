...
test_that("MVP.Data() - vcf", {
    ...
    expect_known_value(geno[], "keep.geno")
    expect_known_value(genoImp[], "keep.genoImp")
    expect_known_value(kinship[], "keep.kinship")
    expect_known_value(pcs[], "keep.pcs")
    expect_known_value(genoInd, "keep.genoInd")
    expect_known_value(map, "keep.map")
})
